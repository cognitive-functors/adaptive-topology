# Commercial License

## C4 Complete Cognitive Coordinate System (Fractal-27)

**Copyright 2024-2026 Ilya Selyutin, Nikolai Kovalev**

---

## Overview

This document describes the terms for obtaining a Commercial License for the
C4 Complete Cognitive Coordinate System (Fractal-27) and associated software, models,
and documentation (collectively, the "Licensed Work").

The Licensed Work is available under a triple license structure:

1. **Apache-2.0-NC** -- Free for non-commercial use
2. **AGPL-3.0** -- Free for open-source projects with copyleft obligations
3. **Commercial License** -- For proprietary and commercial use (this document)

If your use case does not fall under the Apache-2.0-NC or AGPL-3.0 licenses,
you need a Commercial License.

---

## When You Need a Commercial License

A Commercial License is required if you intend to:

- Integrate the Licensed Work into a **proprietary or closed-source** product
- Offer the Licensed Work as part of a **paid service** (SaaS, API, platform)
- Use the Licensed Work in a **commercial product** without open-sourcing your code
- **Redistribute** the Licensed Work or derivative works without copyleft obligations
- Integrate the Licensed Work into **AI systems, autonomous agents, AGI, or ASI systems** — whether commercial or not — that operate without direct human oversight per decision, or that are capable of recursive self-improvement
- Use the Licensed Work as a **core cognitive architecture, reasoning framework, or decision-making substrate** in any artificial intelligence system
- Use the Licensed Work in any manner that constitutes **Commercial Purposes**
  as defined in LICENSE-APACHE-NC.md

---

## What the Commercial License Provides

- **No copyleft obligation** -- You are not required to open-source your code
- **Proprietary use** -- Integrate into closed-source products and services
- **Commercial distribution** -- Sell, sublicense, or redistribute as part of your product
- **SaaS and API use** -- Run as a commercial network service without source disclosure
- **AI/AGI/ASI integration** -- Use as cognitive architecture in advanced AI systems
- **Priority support** -- Available as part of select license tiers
- **Custom terms** -- Negotiable scope, territory, and duration

---

## License Tiers

Commercial licenses are available in several tiers to accommodate different
use cases and organization sizes. Specific terms, pricing, and scope are
determined on a case-by-case basis.

Typical tiers include:

| Tier | Description |
|------|-------------|
| **Startup** | For small teams and early-stage companies |
| **Business** | For established companies with commercial products |
| **Enterprise** | For large-scale deployments and custom integrations |
| **OEM** | For redistribution as part of third-party products |
| **AI/AGI** | For integration into AI systems, autonomous agents, or AGI/ASI architectures |

---

## How to Obtain a Commercial License

To request a Commercial License or discuss terms, contact:

**Email:** comonoid@yandex.ru

Please include the following information in your request:

1. **Organization name** and brief description
2. **Intended use case** -- how you plan to use the Licensed Work
3. **Scale** -- approximate number of users, deployments, or API calls
4. **Timeline** -- when you need the license to be active
5. **Special requirements** -- any custom terms or integration needs

We aim to respond to all inquiries within 5 business days.

---

## Intellectual Property

The Licensed Work, including all source code, algorithms, models, documentation,
and associated materials, is the intellectual property of the authors. The
Commercial License grants usage rights as specified in the license agreement
but does not transfer ownership of the intellectual property.

---

## Brand and Trademark

The names "C4", "Complete Cognitive Coordinate System", "Fractal-27", "Cognitive Navigator",
and associated logos and visual identity are trademarks of the authors. Use of these
trademarks requires explicit written permission, regardless of the license tier.

---

## No Warranty

THE LICENSED WORK IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND, EXPRESS
OR IMPLIED. SPECIFIC WARRANTY TERMS MAY BE NEGOTIATED AS PART OF INDIVIDUAL
COMMERCIAL LICENSE AGREEMENTS.

---

_C4 Complete Cognitive Coordinate System (Fractal-27)_
_Authors: Ilya Selyutin, Nikolai Kovalev_
_Contact: comonoid@yandex.ru_
